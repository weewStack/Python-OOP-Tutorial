# Introduction to Python OOP programing: Practical example part-3
# server library
...
class Book():
    ...
    def bookHTML(self):
        self.html = f"<div class='summaryPack'> \
                        <h2>Title: {self.title}</h2>\
                        <h5>Page count: {self.pageCount}</h5>\
                        <h3>Author(s): {self.AuthorsSTR()}</h3>\
...
class myBookServer(BaseHTTPRequestHandler):
    ...
    def SearchBook(bookname):
        ...
        return data
    # Static Method to insert text into HTML
    @staticmethod
    def UpdateText(OrignalText, TargetInsertText, AddonText):
        wordlen = len(TargetInsertText)
        insertpos = OrignalText.find(TargetInsertText) + wordlen
        NewText = OrignalText[:insertpos]+AddonText+OrignalText[insertpos:]
        return NewText
    ...
    def UpdateHTML(self, DictForm):
        ...
        for item in data['items']:
            try:
                ...
                newHTML += mybook.bookHTML()
            except Exception as e:
                print(e)
        finalHTML = self.UpdateText(HTML, '<div class="summary">', newHTML)
        finalHTML = self.UpdateText(
            finalHTML, '<div class="search-item">', f"Search for the book name {DictForm['btitle']}")
        return finalHTML

    def ResponseError(self, e):
        finalHTML = self.UpdateText(
            HTML, '<div class="search-item">', f"Something went wrong: {e}")
        return finalHTML
    ...
    def do_POST(self):
        ...
        if DictForm['btitle'] == "":
            ...
            try:
                finalHTML = self.UpdateHTML(DictForm)
                self._set_response()
                self.wfile.write(bytes(finalHTML, "utf-8"))
            except Exception as e:
                print(e)
                finalHTML = self.ResponseError(e)
                self._set_response()
                self.wfile.write(bytes(finalHTML, "utf-8"))
